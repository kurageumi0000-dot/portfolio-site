                        ...
                        <div className="bg-white/40 backdrop-blur-xl border border-slate-200/50 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-indigo-100/20 animate-fade-in-up">
                            <div className="divide-y divide-slate-100/50">
                                {newsItems.map((news: News) => (
                                    <Link key={news.id} href={`/news/${news.id}`} className="group block p-8 md:p-10 hover:bg-white/50 transition-all duration-300">
                                        <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-10">
                                            <div className="flex items-center gap-4 shrink-0">
                                                ...
                                                <span className={`px-4 py-1.5 text-xs font-black tracking-widest uppercase rounded-full border ${news.category === 'Important' ? 'bg-rose-50 text-rose-500 border-rose-100' :
